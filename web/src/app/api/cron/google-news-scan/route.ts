...
import {
  runGoogleNewsScan,
  type GoogleNewsScanPhase,
} from "@/lib/social/news-aggregator";

export const maxDuration = 60;

function parsePhase(value: string | null): GoogleNewsScanPhase {
...
async function handle(request: NextRequest) {
  ...
  try {
    const result = await runGoogleNewsScan(phase);
    const status =
    ...
    return NextResponse.json(
      {
        error:
          err instanceof Error ? err.message : "Google News scan failed",
      },
      { status: 500 },
    );
  }
